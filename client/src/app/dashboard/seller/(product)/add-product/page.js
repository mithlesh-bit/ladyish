...
const Page = () => {
  const genAI = new GoogleGenerativeAI(
    "AIzaSyDdjDhNx7I1lmuj17zqvb2yNNHjo27YqLc"
  );
  ...
  const handleGenerateText = async (event) => {
    ...
    const result = await model.generateContent(
      `Write a description for a product named ${title} which has different ${featureTitles}. Make this description concise creative and great for SEO marketing under 120 words. You can use hashtags also `
    );
        ...
        <div className="w-full flex flex-col gap-y-4 p-4 border rounded">
          <label htmlFor="summary" className="w-full flex flex-col gap-y-1">
            ...
            <textarea
              ...
              rows="5"
              maxLength="500"
              onChange={(event) => setSummary(event.target.value)} // Allow users to edit the summary
            />
          </label>
        </div>
        <button
